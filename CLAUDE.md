...
| `XrayConfig`                                   | The data model: which blocks are enabled and what full ARGB color each has (`LinkedHashMap<Block, Integer>`). Owns load/save to a JSON file in the Fabric config dir, writing on every mutation rather than on shutdown. Also owns a `modEnabled` flag (persisted) that gates rendering and chunk scanning without affecting the UI. Old saves that stored a palette index (0–7) are migrated to full ARGB on load.                                                                                               |
...
| `XrayConfigScreen`                             | The in-game config UI. Three panels centered horizontally: Settings (Enable Mod toggle + per-block RGB sliders and hex input, only populated when a block is selected), Enabled (tracked block list with color swatches and remove buttons), Add Blocks (searchable full block list). Selecting a block in Enabled loads it into Settings; the selected row gets a yellow highlight. See its own header comment for the full layout detail.                                                                       |
...
- **The hex field in the Settings panel is not registered via `addDrawableChild`** — it's rendered and hit-tested manually so it's invisible when no block is selected. This means the Screen's keyboard routing (which goes through its registered child list) ignores the field entirely. Two things follow: (1) focus is tracked with an explicit `hexFieldFocused` boolean set in `mouseClicked()`, not via `hexField.isFocused()`, and (2) `hexField.setFocused(true)` must be called immediately before forwarding each `keyPressed`/`charTyped` call, because `TextFieldWidget` guards internally on `isFocused()` — which can silently return false for a widget outside the child hierarchy even after `mouseClicked` set it. Both of these look redundant but aren't.
- **Color saves to disk are deferred 500 ms** after the last slider drag or hex input, so rapid changes don't hammer the disk. `XrayConfig.setColorNoSave()` updates the in-memory map immediately for live preview; `XrayConfig.save()` (made public for this) is called from `render()` once the delay expires. `close()` flushes any pending save synchronously so changes aren't lost if the screen is closed before the delay fires.
